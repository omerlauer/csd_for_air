function KHS = fixed_key_degree(ACS, key_degree, budget, stream)
...
while (budget > 0)
    ...
    % choose fixed number of users sharing random object (if possible)
    sharing_users = find(ACS(:, random_object) == 1);
    random_users = randsample(stream, sharing_users, min([key_degree, ...
        numel(sharing_users)]));
